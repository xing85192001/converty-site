...
export function SurfaceAreaCalculator() {
  ...
  const tSteps = useTranslations("calculator.math.surfaceArea");
          ...
          {surfaceAreaResult.steps.length > 0 && (
            <div className="rounded-lg border bg-muted/50 p-4 space-y-2">
              ...
              {surfaceAreaResult.steps.map((step) => (
                <p
                  key={formatStep(step, tSteps, "calculator.math.surfaceArea")}
                  className="text-sm text-muted-foreground font-mono"
                >
                  {formatStep(step, tSteps, "calculator.math.surfaceArea")}
                </p>
              ))}
            </div>
          )}
