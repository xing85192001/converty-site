...
export function FractionCalculator() {
  ...
  const tSteps = useTranslations("calculator.math.fraction");
          ...
          {fractionResult.steps.length > 0 && (
            <div className="rounded-lg border bg-muted/50 p-4 space-y-2">
              ...
              {fractionResult.steps.map((step) => (
                <p
                  key={formatStep(step, tSteps, "calculator.math.fraction")}
                  className="text-sm text-muted-foreground font-mono"
                >
                  {formatStep(step, tSteps, "calculator.math.fraction")}
                </p>
              ))}
            </div>
          )}
