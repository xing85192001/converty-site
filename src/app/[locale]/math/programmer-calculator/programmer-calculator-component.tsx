...
const useProgrammerCalculatorStore = createCalculatorStore<
	FormValues,
	ProgrammerCalculatorResult
>({
	...
	calculate: (vals) =>
		calculateProgrammerCalculator(vals as ProgrammerCalculatorInput),
});

function repRows(
	rep: { dec: string; hex: string; oct: string; bin: string },
	tR: (key: string) => string,
) {
	return [
		{ label: tR("decBase"), value: rep.dec },
		{ label: tR("hexBase"), value: rep.hex },
		{ label: tR("octBase"), value: rep.oct },
		{ label: tR("binBase"), value: rep.bin },
	];
}
		...
		<div className="space-y-6">
			<div className="grid gap-4 sm:grid-cols-2">
				<div className="space-y-2">
					...
					<div className="flex gap-2">
						...
						<Select
							value={values.aBase}
							onValueChange={(b) => setValue("aBase", b as Base)}
						>
				...
				<div className="space-y-2">
					...
					<div className="flex gap-2">
						...
						<Select
							value={values.bBase}
							onValueChange={(b) => setValue("bBase", b as Base)}
						>
							<SelectTrigger className="w-[140px]">
								<SelectValue />
							</SelectTrigger>
			...
			{result && (
				<div className="space-y-4">
					...
					<ResultGrid results={repRows(result.a, tResults)} columns={4} />
					<p className="text-sm font-medium">{t("reprB")}</p>
					<ResultGrid results={repRows(result.b, tResults)} columns={4} />
					<p className="text-sm font-medium">
						<T k="ui.not-a" />
					</p>
					<ResultGrid results={repRows(result.not, tResults)} columns={4} />
					{result.bitwise && (
						<>
							<p className="text-sm font-medium">
								A {values.bitwiseOp.toUpperCase()} B
							</p>
							<ResultGrid
								results={repRows(result.bitwise, tResults)}
								columns={4}
							/>
						</>
					)}
					{result.shifted && (
						<>
							<p className="text-sm font-medium">
								A {values.shiftDir === "left" ? "<<" : ">>"}{" "}
								{values.shiftAmount}
							</p>
							<ResultGrid
								results={repRows(result.shifted, tResults)}
								columns={4}
							/>
						</>
					)}
