...
export function PermutationCombinationCalculator() {
	...
	const { values, setValue, result, errors, calculationError } =
		usePermutationCombinationStore();
				...
				>
					...
					<SelectContent>
						<SelectItem value="permutation">
							{tMath("permutationNoRepetition")}
						</SelectItem>
						<SelectItem value="combination">
							{tMath("combinationNoRepetition")}
						</SelectItem>
			...
			{pcResult && (
				<div className="space-y-4">
					<OutputDisplay
						...
						value={
							pcResult.result === Infinity
								? "∞"
								: pcResult.result.toLocaleString()
						}
						size="lg"
					/>
							...
							{pcResult.steps.map((step) => (
								<p
									key={formatStep(
										step,
										tSteps,
										"calculator.math.permutationCombination",
									)}
								>
									{formatStep(
										step,
