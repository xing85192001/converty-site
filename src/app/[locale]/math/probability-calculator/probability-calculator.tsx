...
export function ProbabilityCalculator() {
  ...
  const tSteps = useTranslations("calculator.math.probability");
          ...
          {probabilityResult.steps.length > 0 && (
            <div className="rounded-lg border bg-muted/50 p-4 space-y-2">
              ...
              {probabilityResult.steps.map((step) => (
                <p
                  key={formatStep(step, tSteps, "calculator.math.probability")}
                  className="text-sm text-muted-foreground font-mono"
                >
                  {formatStep(step, tSteps, "calculator.math.probability")}
                </p>
              ))}
            </div>
          )}
