...
export default async function PhotoPage({ params }: { params: Promise<{ locale: string }> }) {
  ...
  const t = await getTranslations("nav.photo");
  const tc = await getTranslations("converter");

  const category = getCategoryBySlug("photo")!;
  const converters = getConvertersByCategory("photo");
      ...
      {converters.length > 0 ? (
        <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
          {converters.map((converter) => (
            <Link key={converter.id} href={`/photo/${converter.slug}`}>
        ...
        <div className="text-center py-12 border rounded-lg bg-muted/50">
          <p className="text-muted-foreground">Photo calculators coming soon!</p>
        </div>
      )}
    </div>
  );
