			...
			<div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
				...
				<div className="space-y-2">
					...
					<select
						value={quality}
						onChange={(e) =>
							setQuality(e.target.value as "low" | "typical" | "high")
						}
						className="w-full h-10 px-3 rounded-md border bg-background"
					>
			...
			{result && (
				<div className="space-y-6">
					...
					<ResultGrid
						results={[
							{
								label: tResults("megapixels"),
								value: result.megapixels,
								unit: "MP",
							},
							{
								label: tResults("totalPixels"),
								value: result.totalPixels.toLocaleString(),
							},
							{
								label: tResults("bitsPerPixel"),
								value: result.bitsPerPixel,
								unit: "bpp",
							},
							{
								label: tResults("sizeKB"),
								value: result.estimatedKB,
								unit: "KB",
							},
						]}
						columns={2}
						...
						<table className="w-full text-sm">
							...
							<tbody>
								{IMAGE_FORMATS.map((f) => {
									...
									const estFormatted = estResult.ok
										? estResult.value.formatted
										: "";
									return (
										<tr
											key={f.id}
