...
const HoursCalculator = dynamic(
  () => import("./hours-calculator").then((mod) => mod.HoursCalculator),
  {
...
export async function generateMetadata({
  ...
  const t = await getTranslations({ locale, namespace: "converter.hours" });
...
export default async function HoursPage({ params }: { params: Promise<{ locale: string }> }) {
  ...
  const t = await getTranslations("converter.hours");
