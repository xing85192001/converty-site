		...
		<div className="space-y-6">
			...
			{result && (
				<div className="space-y-6">
					<ResultGrid
						results={[
							{ label: tResults("width"), value: result.width, unit: "in" },
							{ label: tResults("height"), value: result.height, unit: "in" },
							{
								label: tResults("diagonal"),
								value: result.diagonal,
								unit: "in",
							},
							{ label: tResults("area"), value: result.area, unit: "sq in" },
						]}
						columns={2}
					/>
						...
						<table className="w-full text-sm">
							...
							<tbody>
								{COMMON_DIAGONALS.map((d) => {
									const rResult = calculateScreenSize(
										d.size,
										parseInt(aspectW),
										parseInt(aspectH),
									);
