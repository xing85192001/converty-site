					...
					{RESOLUTIONS.map((res) => (
						<button
							...
							onClick={() => {
								...
								const preset =
									BITRATE_PRESETS[res.id as keyof typeof BITRATE_PRESETS];
								if (preset) {
			...
			{result && (
				<div className="space-y-6">
					...
					<ResultGrid
						results={[
							{
								label: tResults("totalSize"),
								value: result.totalMB,
								unit: "MB",
							},
							{
								label: tResults("totalSize"),
								value: result.totalGB,
								unit: "GB",
							},
							{
								label: tResults("videoData"),
								value: Math.round(result.videoBytes / (1024 * 1024)),
								unit: "MB",
							},
							{
								label: tResults("audioData"),
