...
async function fetchToBlobURL(
  ...
  // The self-hosted /ffmpeg copy is stored gzip-compressed because Cloudflare
  // Pages caps single files at 25 MB while the raw ffmpeg-core.wasm is ~32 MB.
  // Browsers only auto-decompress with `Content-Encoding: gzip`, which the
  // Pages asset server does not add here, so decompress manually when the
  // magic bytes (0x1f 0x8b) indicate gzip. If decompression fails, keep the
  // original bytes — the wasm-magic check in loadFfmpegCore will reject them
  // and the caller falls back to the next host.
  if (buf.length > 2 && buf[0] === 0x1f && buf[1] === 0x8b) {
    try {
      ...
      /* keep original bytes; magic check will reject */
    }
  }
...
export function VideoWatermarkRemover() {
  ...
  // Edge/Chromium sometimes does not paint the first frame of a freshly created
  // blob URL until the element is explicitly reloaded. Kick the video element
  // whenever the source changes and nudge the current time so the canvas/RGB
  // buffer is actually produced.
  useEffect(() => {
    const video = videoRef.current;
    if (!originalUrl || !video) return;
    video.load();
  }, [originalUrl]);
              ...
              >
                ...
                <video
                  ...
                  onCanPlay={(e) => {
                    const v = e.currentTarget;
                    setVideoReady(true);
                    // Force a tiny seek so Chromium/Edge actually paints the
                    // first frame instead of leaving the canvas grey.
                  ...
                  onError={(e) => {
                    ...
                    setError(`Video load failed (code ${code}: ${msg}). Try an MP4/H.264 file.`);
                  }}
